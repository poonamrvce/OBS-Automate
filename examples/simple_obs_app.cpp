...
obs_source_t*  setSceneAndSource() {
	...
	if (!obs_transition) {
		LOG(ERROR)<<"Error while creating obs_transition %s", "cut_transition");
		return NULL;
	}*/
	...
	if (!obs_scene) {
		LOG(ERROR)<<"Error while creating obs_scene");
		return NULL;
	}*/
	...
	if (!obs_data) {
		LOG(ERROR)<<"Failed to create obs_data"<<std::endl;
		return NULL;
	}

	LOG(DEBUG)<<"create ffmpeg src"<<std::endl;

	obs_data_set_string(obs_data, "local_file", "/vol/sample/krishna.mp4");
	...
	if (!obs_source) {
		LOG(ERROR)<<"Failed to create obs_source"<<std::endl;
		return NULL;
	}
	...
	if (!obs_scene_item) {
		LOG(ERROR)<<"Error while adding scene item");
		return NULL;
	}*/
	...
	LOG(DEBUG)<<"obs_transition_set"<<std::endl;
...
int studioInit() {
	...
	obs_data_set_string(rtmp_settings, "key", "app1");
	...
	// if (!output) {
    //     LOG(ERROR)<<"Couldn't create output")
    //     return -1;
	// }
	...
	if (!enc_a) {
        LOG(ERROR)<<"Couldn't create enc_a"<<std::endl;
        return -1;
	}

	enc_a_settings = obs_encoder_get_settings(enc_a);
	if (!enc_a_settings) {
        LOG(ERROR)<<"Failed to create enc_a_settings"<<std::endl;
        return -1;
	}
	...
	if (!enc_v) {
        LOG(ERROR)<<"Couldn't create enc_v"<<std::endl;
        return -1;
	}

	enc_v_settings = obs_encoder_get_settings(enc_v);
	if (!enc_v_settings) {
        LOG(ERROR)<<"Failed to create enc_v_settings"<<std::endl;
        return -1;
	}
